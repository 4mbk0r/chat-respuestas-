...
class Conexion {
    ...
    respuesta(numero) {
        ...
        var db = firebase.firestore();
        var docRef = db.collection("BFQ").doc(idpregunta+"");
        ...
        docRef.get().then((doc) => {
            ...
            if (doc.exists) {
                ...
                opciones.forEach(element => {
                    var mensajeinicio = '<input type="radio" class="btn-check pulse" name="options" value="'+ valor +'" id="'+idpregunta+'-'+valor+'" autocomplete="off">'
                    mensajeinicio+='<label class="btn btn-danger" for="'+idpregunta+"-"+valor+'">'
    ...
    escribir(n_pregunta, respuesta) {
        console.log("numero de pregunta "+ n_pregunta);
        var db = firebase.firestore();
        var docRef = db.collection("BFQ").doc(n_pregunta);
